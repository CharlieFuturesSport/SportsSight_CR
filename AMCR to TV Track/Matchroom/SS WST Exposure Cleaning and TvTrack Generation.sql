...
-- CREATE TABLE OF EVENTS (if it doesn't seem to be working, ensure that there is a / at the end of each event name)
...
-- UPDATE FOR STATIC TO TABLESIDE BRANDING
	update Toolkit_AzureModels_CombinedResults
	set asset = 'Tableside Branding'
	WHERE EVENT + '/' in (select sportsevent from #SportEvents)
	and (asset = 'Static Board Signage' or asset = 'Static Signage')
		...
		(
			...
			FROM 
				(
					...
					AND asset <> 'Exclude'
					AND (Sport = @sport)
					AND ModelType IN ('OCR', 'OCR_Coords')
				) AS OCRresults
			GROUP BY
				Event, [Filename], Brand, Asset
		) OCR
